`ifndef CFS_ALGN_VIRTUAL_SEQUENCE_REG_ACCESS_RANDOM_SV
    ...
    class cfs_algn_virtual_sequence_reg_access_random 
        ...
        virtual task body();
           ...
           p_sequencer.model.reg_block.get_registers(registers);
            ...
            `uvm_info("DBG", $sformatf(
                "reg_block has %0d registers", registers.size()
            ), UVM_NONE)
                ...
                case (access)
                    ...
                    default: begin
                       `uvm_fatal("ALGORITHM_ISSUE", $sformatf(
                            "Unsupported value for access: %0s",
                            access.name()
                       )) 
                    end
                endcase
        ...
        protected virtual function uvm_access_e get_random_access(
            uvm_reg register, int unsigned access_idx
        );
            uvm_access_e result;
